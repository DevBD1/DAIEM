...
const validatePdf = async (fileName: string): Promise<{ isValid: boolean; error?: string }> => {
    try {
...
export const getPdfUri = async (fileName: string): Promise<PDFInfo> => {
    try {
        ...
        // Load from assets
        const asset = Asset.Asset.fromModule(pdfAssets[fileName as keyof typeof pdfAssets]);
